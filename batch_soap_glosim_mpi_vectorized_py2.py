...
def get_property_from_json(file_name, property_name):
    data = read_json(file_name)
    return data['properties'][property_name]

def binding_energy(nmpc, total_energy, single_molecule_energy):
    return float(total_energy) - (float(nmpc) * float(single_molecule_energy))
...
def create_xyz_file(struct_file, single_molecule_energy, structs_fpath_for_rank, energy_name='energy'):
    '''
    struct_file: str
        path to json file containing a structure
    single_molecule_energy: float
        total energy of the single molecule (eV)
    structs_fpath_for_rank: str
        path to write the xyz structure to (.xyz extension)
    energy_name: str
        In the properties dictionary in the structure dictionary, this is the key for the total energy of the structure (eV)

    Return:
        None

    Purpose: Append a structure from a .json file to a .xyz file that will possibly contain many structures and will be passed into quippy
    '''
    ...
    species_list = [struct['geometry'][i][3] for i in range(len(struct['geometry']))]
    ...
    if 'properties' in struct and energy_name in struct['properties']:
        total_energy = struct['properties'][energy_name]
    else:
        total_energy = 'none'
    if total_energy == 'none':
        energy_str = 'energy=' + total_energy
    else:
        energy_str = 'energy=' + str(normalized_BE_by_napc(napc, nmpc, total_energy, single_molecule_energy))
    lattice_vecs_str = 'Lattice=' + '"' + str(lattice_vector_a[0]) + ' ' + str(lattice_vector_a[1]) + ' ' + str(lattice_vector_a[2]) + ' ' + str(lattice_vector_b[0]) + ' ' + str(lattice_vector_b[1]) + ' ' + str(lattice_vector_b[2]) + ' ' + str(lattice_vector_c[0]) + ' ' + str(lattice_vector_c[1]) + ' ' + str(lattice_vector_c[2]) + '"'
    ...
    file_utils.write_lines_to_file(structs_fpath_for_rank, xyz_lines, mode='a')
    return napc
...
def rank_print(rank, *print_message):
    with open('output_' + str(rank) + '.out', mode='a') as f:
        for i, entry in enumerate(print_message):
            f.write(str(entry))
            if i != len(print_message) - 1:
                f.write(' ')
        f.write('\n')
...
def delete_some_unnecessary_matrices(loaded_soaps, matrices_to_keep, upperbound_bytes):
    ...
    rank_print(comm.rank, 'initial available memory', available_memory)
    while available_memory < upperbound_bytes:
        ...
        rank_print(comm.rank, 'available memory', available_memory, 'is less than the desired threshold', upperbound_bytes)
        rank_print(comm.rank, 'number of matrices loaded', len(loaded_soaps), 'nbytes', sum([loaded_soaps[loaded_soap].nbytes for loaded_soap in loaded_soaps]) / 1e9, 'GB')
        for loaded_soap in loaded_soaps:
            if loaded_soap not in matrices_to_keep:
                rank_print(comm.rank, 'deleting', loaded_soap)
                del loaded_soaps[loaded_soap]
                break
        available_memory = dict(psutil.virtual_memory()._asdict())['available']
    rank_print(comm.rank, 'available memory', available_memory, 'is likely sufficient to load another matrix.')


def load_soaps(task_idx, loaded_soaps, task_list, num_atoms_arr, global_envs_dir, lowmem, breathing_room_factor, params):
    '''
    task_idx: int
    ...
    if len(loaded_soaps) == 0:
        ...
        params.one_matrix_len = loaded_soaps[str(i)].shape[0]
        params.one_matrix_nbytes = loaded_soaps[str(i)].nbytes
    #for loaded_soap in loaded_soaps:
    #    rank_print(comm.rank, 'loaded_soap', loaded_soap, 'loaded_soaps[loaded_soap].shape', loaded_soaps[loaded_soap].shape, 'loaded_soaps[loaded_soap].nbytes', loaded_soaps[loaded_soap].nbytes)
    float32_size = 4
    num_atoms_in_i = int(num_atoms_arr[i])
    #print('num_atoms_in_i', num_atoms_in_i)
    num_atoms_in_j = int(num_atoms_arr[j])
    rank_print(comm.rank, 'params.one_matrix_nbytes', params.one_matrix_nbytes, 'params.one_matrix_len', params.one_matrix_len, num_atoms_in_i, num_atoms_in_j)
    bytes_per_tiled_matrix = params.one_matrix_nbytes * float(params.one_matrix_len) * max(num_atoms_in_i, num_atoms_in_j) / min(num_atoms_in_i, num_atoms_in_j)
    rank_print(comm.rank, 'bytes_per_tiled_matrix', bytes_per_tiled_matrix / 1e9, 'GB')
    ...
    if str(i) + '_repeat_' + str(num_atoms_in_j) not in loaded_soaps:    
        if str(i) not in loaded_soaps:
            if lowmem:
                upperbound_bytes = bytes_per_tiled_matrix * breathing_room_factor / float(num_atoms_in_i)
                delete_some_unnecessary_matrices(loaded_soaps, matrices_to_keep, upperbound_bytes)
            rank_print(comm.rank, 'about to load ' + str(i))
            loaded_soaps[str(i)] = np.load(os.path.join(global_envs_dir, 'atomic_envs_matrix_' + str(i) + '.npy'))
            rank_print(comm.rank, 'loaded ' + str(i))
        if lowmem:
            upperbound_bytes = bytes_per_tiled_matrix * breathing_room_factor
            delete_some_unnecessary_matrices(loaded_soaps, matrices_to_keep, upperbound_bytes)
        #print('loaded_soaps['+str(i)+']', loaded_soaps[str(i)])
        rank_print(comm.rank, 'about to load ' + str(i) + '_repeat_' + str(num_atoms_in_j))
        loaded_soaps[str(i) + '_repeat_' + str(num_atoms_in_j)] = np.repeat(loaded_soaps[str(i)], num_atoms_in_j, axis=0).flatten()
        rank_print(comm.rank, 'loaded ' + str(i) + '_repeat_' + str(num_atoms_in_j))
    ...
    if str(j) + '_tile_' + str(num_atoms_in_i) not in loaded_soaps:
        if str(j) not in loaded_soaps:
            if lowmem:
                upperbound_bytes = bytes_per_tiled_matrix * breathing_room_factor / float(num_atoms_in_j)
                delete_some_unnecessary_matrices(loaded_soaps, matrices_to_keep, upperbound_bytes)
            rank_print(comm.rank, 'about to load ' + str(j))
            loaded_soaps[str(j)] = np.load(os.path.join(global_envs_dir, 'atomic_envs_matrix_' + str(j) + '.npy'))
            rank_print(comm.rank, 'loaded ' + str(j))
        if lowmem:
            upperbound_bytes = bytes_per_tiled_matrix * breathing_room_factor
            delete_some_unnecessary_matrices(loaded_soaps, matrices_to_keep, upperbound_bytes)
        #rank_print(comm.rank, 'num_atoms_in_i', num_atoms_in_i, 'type(num_atoms_in_i)', type(num_atoms_in_i))
        rank_print(comm.rank, 'about to load ' + str(j) + '_tile_' + str(num_atoms_in_i))
        loaded_soaps[str(j) + '_tile_' + str(num_atoms_in_i)] = np.tile(loaded_soaps[str(j)], (num_atoms_in_i, 1)).flatten()
        rank_print(comm.rank, 'loaded ' + str(j) + '_tile_' + str(num_atoms_in_i))
    ...
    for params_set in params_combined_iterable:
        ...
        if comm.rank == 0:
            ...
            while True:
                ...
                rank_print(comm.rank, 'took', time.time() - recv_start_time, 'secs to recv message', message)
                # read_rank received message from comm.rank 0
                if (isinstance(message, str) and message == 'done') or len(message) == 0:
                    break
                ...
                structs_fpath_for_rank = os.path.join(os.path.dirname(xyz_fpath), file_utils.fname_from_fpath(xyz_fpath) + '_' + str(start) + '_' + str(stop) + '.xyz')
                for struct_idx in message:
                    create_xyz_file(data_files[struct_idx], params.single_molecule_energy, structs_fpath_for_rank, energy_name='energy')
                al = quippy.AtomsList(structs_fpath_for_rank)
        ...
        if comm.rank == 0:
            ...
            traversal_order_start_time = time.time()
            incomplete_tasks = get_traversal_order(incomplete_tasks, kernel_calculation_path)
            print('got traversal order in', time.time() - traversal_order_start_time, 'secs')
            print('incomplete_tasks takes', sys.getsizeof(incomplete_tasks) / 1000000000.0, 'GB')
            done_ranks = []
            while len(done_ranks) != comm.size - 1:
                # world rank 0 waiting for ready rank
                ready_rank = comm.recv(source=MPI_ANY_SOURCE, tag=0)
                print('rank', ready_rank, 'is ready for kernel calculation')
                # a rank is now ready to recieve a message
                if len(incomplete_tasks) != 0:
                    ...
                    done_ranks.append(ready_rank)
                # world rank 0 sending message to ready_rank
                print('sending message', message, 'to ready rank for kernel calculation')
                comm.send(message, dest=ready_rank, tag=1)
                print('sent message', message, 'to ready rank for kernel calculation')
                # world rank 0 sent message to ready_rank
        else:
            loaded_soaps = {}
            num_atoms_arr = np.load(os.path.join(kernel_calculation_path, 'num_atoms_arr.npy'))
            while True:
                ...
                recv_start_time = time.time()
                message = comm.recv(source=0, tag=1)
                rank_print(comm.rank, 'time for recv', time.time() - recv_start_time)
                rank_print(comm.rank, 'got message', message, 'for kernel calculation')
                # read_rank received message from comm.rank 0
                ...
                breathing_room_factor = float(comm.size + 1)
                for task_idx in range(len(message)):
                    num_atoms_in_i, num_atoms_in_j = load_soaps(task_idx, loaded_soaps, message, num_atoms_arr, global_envs_dir, params.lowmem, breathing_room_factor, params)
                    ...
                    rank_print(comm.rank, 'wrote kernel value for i, j = ', i, j)
