...
single_molecule_energy = -8357.500268932
#input_str is path to folder with jsons with energies
input_str = '../AP/AP_exemplars_' + str(num_structs)
scpt_dir = os.path.dirname(os.path.abspath(__file__))
input_dir = os.path.join(scpt_dir, input_str)
output_dir = os.path.join(scpt_dir, str(nmpc) + 'mpc_xyz')

if not os.path.exists(output_dir):
    os.mkdir(output_dir)

outfile_name = os.path.join(output_dir, 'target1_' + str(nmpc) + 'mpc_SCF_energies_AP_' + str(num_structs) + '_but_give_BE.xyz')
...
for data_file in data_files:
    ...
    if (en - GM_energy < energy_cutoff):
        energy_str = 'energy=' + str(binding_energy(nmpc, en, single_molecule_energy))
        print(binding_energy(nmpc, en, single_molecule_energy))
