...
def write_test_xyz_file(test_idx_lines, wdir_path):
    '''
    test_idx_lines: list
        list of lines to write to test.xyz
    wdir_path: str
    ...
    Purpose: write test.xyz to wdir_path. It is assumed that train.xyz
        is already there.
    '''
    with open(os.path.join(wdir_path, 'test.xyz'), 'w') as f:
        f.writelines(test_idx_lines)

def write_test_en_dat(wdir_path):
    '''
    wdir_path: str
        path to directory to write the energy dat files to. This
        is the same directory as contains the xyz files.
...
def delete_any_previous_tests_in_this_dir(path):
    '''
    path: str
        path to directory where previous tests may have been carried out

    Purpose: Delete any specific files that correspond to previous tests
    '''
    fnames_to_remove = ['test_results', 'test.xyz*', '*test*.dat', '*rect*k', 'log_ref', 'lockfile']
    fpaths_to_remove = [os.path.join(path, fname) for fname in fnames_to_remove]
    file_utils.rms(fpaths_to_remove)

def create_new_test(inst, owd):
    '''
    inst: contains conf file params
                ...
                for train_num_structs in inst.get_list(sname, 'train_num_structs'):
                    ...
                    for test_num in range(int(inst.get(sname, 'test_num'))):
                        ...
                        for train_num in range(int(inst.get(sname, 'train_num'))):
                            ...
                            '''
                            delete_any_previous_tests_in_this_dir(train_num_path)
                            write_test_xyz_file(test_idx_lines, train_num_path)
                            write_test_en_dat(train_num_path)
