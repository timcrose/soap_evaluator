...
def get_traversal_order(incomplete_tasks, num_atoms_arr):
    '''
    ...
    num_atoms_arr: np.array shape (num_structures,)
        1D array containing the number of atoms in each structure.
    ...
    for params_set in params_combined_iterable:
        ...
        if comm.rank == 0:
            ...
            while len(done_ranks) != comm.size - 1:
                ...
                ready_rank, partial_num_atoms_lst = comm.recv(source=MPI_ANY_SOURCE, tag=0)
                #print('ready_rank, partial_num_atoms_lst', ready_rank, partial_num_atoms_lst)
                num_atoms_lst = num_atoms_lst + partial_num_atoms_lst
            ...
            num_atoms_lst.sort()
            num_atoms_arr = np.array(deepcopy(num_atoms_lst))[:,1]
            del num_atoms_lst
            ...
            num_atoms_arr = None
            partial_num_atoms_lst = []
            while True:
                comm.send((comm.rank, partial_num_atoms_lst), dest=0, tag=0)
                ...
                # Construct xyz file for structures in message and get the number of atoms in their unit cells for future use.
                partial_num_atoms_lst = [[struct_idx, create_xyz_file(data_files[struct_idx], params.single_molecule_energy, structs_fpath_for_rank, energy_name='energy')] for struct_idx in message]
        ...
        num_atoms_arr = comm.bcast(num_atoms_arr, root=0)
        start_time_kernel = time.time()
        root_print(comm,'time to write xyz files and calculate environments', start_time_kernel - start_time_envs)
        ...
        incomplete_tasks = get_traversal_order(incomplete_tasks, num_atoms_arr)
        if comm.rank == 0:
            done_ranks = []
            while len(done_ranks) != comm.size - 1:
                # world rank 0 waiting for ready rank
