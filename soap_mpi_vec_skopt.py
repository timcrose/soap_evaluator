...
from python_utils.list_utils import sort_by_col
...
if comm_world.rank == 0:
    ...
    num_replicas = float(sys.argv[-1])
    num_ranks_per_replica = int(comm_world.size / num_replicas)
    if num_ranks_per_replica != (comm_world.size / num_replicas):
...
def get_weighted_avg_test_R2s(avg_test_R2s):
    '''
    avg_test_R2s: 1D np.array of float
        Array of R^2 values averaged over ntests. Each element belongs to a different sampling method.
    
    Return:
        weighted_avg_test_R2: float

    Purpose: For a given hyperparameter set, I might do multiple sampling methods such as iss, random, fps...
        However, I need one objective function value to tell to skopt.Optimizer. One way is to get the avg R^2
        value from each of these sampling methods and create an overall score which is the weighted avg of
...
def soap_workflow(params):
    ...
    num_param_combos = inst.get('master', 'num_param_combos', eval=True)
    if num_param_combos is None:
        ...
        opt = Optimizer([Integer(4,12), Integer(4,12), Real(2.0, 40.0), Real(0.3, 40.0), Real(0.1, 10.0)], n_initial_points=50)
    ...
    for params_iter in iterable:
        ...
        if num_param_combos is None:
            ...
            if comm_world.rank == 0:
                ...
                params_sets = None
            params_set = comm.scatter(params_sets, root=0)
        for i, p in enumerate(params_to_get):
            param_string += p
        ...
        if num_param_combos is not None:
            ...
            avg_test_R2s = comm.gather(avg_test_R2, root=0)
            if comm.rank == 0:
                weighted_avg_test_R2s = get_weighted_avg_test_R2s(avg_test_R2s)
                opt.tell(params_sets, weighted_avg_test_R2s)
